...
namespace SHCore
{
	...
	protected:
		FRenderWindowOption RenderWindowOption;
	};

	class SH_CORE_API SHRenderWindowFactory
	{
	public:
		static SHRenderWindow::FRenderWindowOption CreateRenderWindowOption(
			SHRenderWindow::EWindowMode inWindowMode = SHRenderWindow::EWindowMode::Window,
			int32 inWidth = 1280, int32 inHeight = 720,
			int32 inPosX = 0, int32 inPosY = 0,
			const std::string& inWindowTitle = "")
		{
			return SHRenderWindow::FRenderWindowOption(inWindowMode, inWidth, inHeight, inPosX, inPosY, inWindowTitle);
		}
	};
}